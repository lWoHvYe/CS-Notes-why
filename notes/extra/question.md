...
- 如果工作中无法遇到某些场景，就通过学习（培训、网课）的方式来获取
...
- 点赞系统设计
    ...
    - 支持高并发
        ...
        - 当先本地存储，再定期聚合时，因为执行聚合期间仍会有新的数据加入，如何保证一致性的问题：
            - 可以采用全局锁或分区锁，但会影响性能
            - 可以考虑双缓冲机制，开始聚合前，写入侧切换到另一缓冲，这里需考虑切换的问题
            - 可以更近一步的使用流处理框架Flink、Spark、Storm，这种感觉好像不错，说实话
      ```
      用户A点赞的帖子:
      帖子1 -1 
      帖子2 1 
...
- 秒杀系统设计
    - 系统总体是分层设计。针对总量1000，可以分成10个集群。每个集群预存110个商品（有冗余）。
    - 第一层是网关层，只做路由，将请求按相关的Rule Route到下层集群。理论上可基于lua设置一个flag，Open后表示秒杀结束直接返回。
    - 第二层是各集群的Portal Gateway，基于lua做Route及预存商品的扣减，该层可预存与总量一至的库存数（直接放入配置中，可以是 总量*某个系数，这里需要数倍于该集群配额的预存库存数）。扣减完后直接返回。
    - 第三层承接Request并将其发到MQ，做削峰处理，可加入限流Ratelimt、熔断Circuit-Breaker
    - 第四层Consumer，消费Msg，更新预存库存数（配置在Redis中，每个预存数量为 总数M/集群数N + 20%冗余），这里也可像上面购票系统一样做Redis分区锁，每个分区预存量为该集群配额/分区数
    - 第五层为第四层的业务侧，多层filter后，该层流量并发都适度，做实际的下单、扣减实际库存等逻辑，使用分布式锁。
    - 第六层是optional，该层是数据层Data（未必是DB也可以是Cache），该层存储实际的商品SKU（无冗余），可多个上层集群使用一个，理论上不会有少卖的情况，因为上层是多个集群每个集群还有冗余节点用于切换。
    - 第七层是DB，存储商品、订单等信息，这个可由上层异步的落库，减轻瓶颈。
    - 最后可能还需要一个Task来更新第一层网关中的flag。
    - 理想情况下下层的scope会覆盖上层的冗余节点（用于切换），所以上层的服务节点和冗余节点在同一个分布式锁内。（主要是第五、第六层，比如第五层每个集群配120，两个第五层共用第六层，第六层就可以配200），虽然层层拆分，但最终数据需要汇总，第六层的作用便是缓冲。
