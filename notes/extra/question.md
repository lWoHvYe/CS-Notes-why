...
- 秒杀系统设计
    ...
    - 第二层是各集群的Portal Gateway，基于lua做Route及预存商品的扣减，该层可预存与总量一至的商品数（直接放入配置中）。扣减完后直接返回。
    - 第三层承接Request并将其发到MQ，做削峰处理，可加入限流Ratelimt、熔断Circuit-Breaker
    - 第四层Consumer，消费Msg，更新预存商品数（配置在Redis中，每个预存数量为 总数M/集群数N + 20%冗余），这里也可像上面购票系统一样做Redis分区锁，每个分区预存量为该集群配额/分区数
