...
- 拼写纠错：编辑距离
- 简单整合：使用Spring Data Elasticsearch整合，简化交互。通过继承ElasticsearchRepository接口，这样就拥有了一些基本的Elasticsearch数据操作方法，同时可定义一些衍生查询方法。通过使用ElasticsearchRestTemplate的search()
  方法进行复杂查询。
...
- 读写策略：旁路缓存模式、读写穿透、异步缓存写入。先删除Cache还是先更新DB，当下还不确定(每种都有自身的问题)，可以先更新DB再延迟双删缓存。
